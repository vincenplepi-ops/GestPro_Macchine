...
module.exports = async function handler(req, res) {
  try {
    ...
    if (!containsScanner) {
      ...
      res.setHeader('X-GestPro-Scanner', 'v20-iphone-ocr-fix');
      return res.status(200).send(js);
    }
    // Stop before bi(): that helper must remain in the bundle because xi()
    // uses it to order the four page corners.
    js = js.replace(/function yi\(e\)\{[\s\S]*?(?=function bi\()/, SAFARI_SAFE_WORKING_CANVAS);
    ...
    if (js === original || !js.includes('2200/n') || !js.includes('function bi(e)') || !js.includes('VPtemplateBands') || !js.includes('VPextraOCR') || !js.includes('Math.max(n,10)') || !js.includes('n.push({id:e.key') || !js.includes('start:a.value,end:o.value')) {
